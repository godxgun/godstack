...
- **Term** — bytes in, grid out. No OS, no GPU, no PTY, no Peak include. Never `#ifdef` a brand.
...
- Callers test the result, not the OS. New OS `#ifdef` belongs in Peak (or a Rend backend), not in Term or in a Peak caller.
